...
export default function WebCategoriePage({ params }: { params: Promise<{ categorie: string }> }) {
  ...
  const [form, setForm] = useState({ url: '', contenu: '', tags: '' });
  const [saving, setSaving] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/content/web?categorie=${encodeURIComponent(categorie)}`);
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Link href="/web" className="p-2 rounded-xl hover:bg-slate-100 transition-colors">
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-2xl p-16 flex flex-col items-center justify-center text-slate-400 space-y-3">
          ...
          <p className="font-semibold">Aucune ressource dans ce dossier</p>
            ...
            >
              <div className={`bg-gradient-to-br ${style.gradient} p-5 flex items-center justify-between`}>
                <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-white/20 text-white truncate max-w-[160px]">
                  {item.source || (item.url ? new URL(item.url).hostname : 'Web')}
                </span>
                {item.url && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h2 className="text-lg font-bold text-slate-900">Ajouter — {categorie}</h2>
            ...
            <form onSubmit={handleAdd} className="p-6 space-y-5">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">URL <span className="text-slate-400 font-normal">(optionnel)</span></label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Contenu / Notes <span className="text-slate-400 font-normal">(optionnel si URL fournie)</span></label>
                <textarea rows={4} value={form.contenu} onChange={e => setForm(f => ({ ...f, contenu: e.target.value }))}
                  placeholder="Collez un extrait ou vos notes..."
                ...
                <p className="text-xs text-slate-400 mt-1">Claude génère titre + résumé automatiquement.</p>
              </div>
              <div>
                ...
                <input type="text" value={form.tags} onChange={e => setForm(f => ({ ...f, tags: e.target.value }))}
                  placeholder="article, étude, outil"
                  className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
              <button type="submit" disabled={saving || (!form.url && !form.contenu)}
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {selected.tags?.map(tag => (
                  <span key={tag} className={`text-xs px-2.5 py-1 rounded-full font-medium ${style.badge} ${style.badgeText}`}>{tag}</span>
                ))}
